...
### class
grep -E -A3 "^<http://sybig.de/tfclass#\d+.\d+>"  tfclass.ttl |\
  grep -vE '\"Class\"' |perl -pe 's/;\n/>\t/mg' |\
  perl -ne '/(\d+.\d+)(.*)"(.*)"(.*)"(.*)"/&&print "$1\t$3\t$5\n"' > class.txt

grep -E '^<http://sybig.de/tfclass#3.7>'  tfclass.ttl -A3|\
  grep -vE '\"Class\"' |perl -pe 's/;\n/>\t/mg' |\
  perl -ne '/(\d+.\d+)(.*)"(.*)"/&&print "$1\t$3\t \n"' >>class.txt

grep -E '^<http://sybig.de/tfclass#6.7>'  tfclass.ttl -A3|\
  grep -vE '\"Class\"' |perl -pe 's/;\n/>\t/mg' |\
  perl -ne '/(\d+.\d+)(.*)"(.*)"/&&print "$1\t$3\t \n"' >>class.txt

### superclass
grep -E -A3 "^<http://sybig.de/tfclass#\d+>"  tfclass.ttl |\
  grep -vE '\"Class\"' |perl -pe 's/;\n/>\t/mg' |\
  perl -ne '/(\d+)(.*)"(.*)"(.*)"(.*)"/&&print "$1\t$3\t$5\n"' > superclass.txt
